...
# Patch kaldi-native-fbank and kaldifst for Android API 19 (rand_r not available in Bionic before API 21)
for math_file in \
    "$CACHED_DEPS/kaldi_native_fbank-src/kaldi-native-fbank/csrc/kaldi-math.cc" \
    "$CACHED_DEPS/kaldifst-src/kaldifst/csrc/kaldi-math.cc"; do
    if [ -f "$math_file" ]; then
        sed -i 's/#if !defined(_POSIX_THREAD_SAFE_FUNCTIONS)/#if 1/g' "$math_file"
    fi
done
...
cmake -GNinja -B$BUILD_DIR \
    ...
    -DCMAKE_CXX_FLAGS_RELEASE="-O3 -DNDEBUG -U_POSIX_THREAD_SAFE_FUNCTIONS" \
    -DCMAKE_C_FLAGS_RELEASE="-O3 -DNDEBUG -U_POSIX_THREAD_SAFE_FUNCTIONS" \
